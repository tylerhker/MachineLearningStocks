...
def parse_keystats_avg(sp500_df, stock_df):
    """
    ...
    for stock_directory in tqdm(stock_list, desc="Parsing progress:", unit="tickers"):
        ...
        for file in keystats_html_files:
            ...
            try:
                ...
                stock_1y_price = stock_df.loc[one_year_later, ticker.upper()]
                stock_1y_price_all = stock_df.loc[current_date:one_year_later, ticker.upper()]
                stock_year_avg_price = sum(stock_1y_price_all)/len(stock_1y_price_all)
                # print(stock_year_avg_price)
            except KeyError:
            ...
            stock_p_change_year_avg = round(
                ((stock_year_avg_price-stock_price)/stock_price * 100), 2
            )
    ...
    # Output the CSV
    df.to_csv("keystats_avg.csv", index=False)
