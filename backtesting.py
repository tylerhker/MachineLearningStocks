...
def backtest_data(method='sp500'):
    """
    ...
    features = data_df.columns[8:] #avg method
    ...
    elif method == 'self':#self compare
        print(f'Using {method} method')
        y = list(
           status_calc_self(
               data_df["stock_p_change"], outperformance=10
               )
           )
    elif method == 'avg':#avg method
        print(f'Using {method} method')
        y = list(
           status_calc(
               data_df["stock_p_change_year_avg"], data_df["SP500_p_change_year_avg"], outperformance=1
               )
           )
    ...
    return X_train, X_test, y_train, y_test, z_train, z_test, ticker_train, ticker_test

def backtest_train(X_train, X_test, y_train, y_test, z_train, z_test, ticker_train, ticker_test):
...
if __name__ == "__main__":
    X_train, X_test, y_train, y_test, z_train, z_test, ticker_train, ticker_test = backtest_data(method='avg')
    # model_grid_search(X_train, y_train)
    backtest_train(X_train, X_test, y_train, y_test, z_train, z_test, ticker_train, ticker_test)
